...
(g <- visyn %>% ggplot(aes(movementTime, trial, group = condition)) + 
  ...
  scale_y_continuous(name = "Trial Number", 
                     breaks = c(1:20)) +
  labs(tag = "A") +
  scale_shape_manual(values = c("black", "#3300CC", "#FFD700")) +
  theme_bw() + #sets the theme to plane
  theme(panel.border = element_blank(), 
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), 
...
## Velocity
(i <- visyn %>% ggplot(aes(velocity, trial, group = condition)) + 
  geom_point(size = 4, shape = 16,
             aes(color = condition)) +
  scale_x_continuous(name = "Velocity at Release (mph)", 
                     limit = c(20, 55), 
...
## Release Time
(h <- visyn %>% ggplot(aes(movementTime, group = condition, fill = condition)) + 
...
## Velocity
(j <- visyn %>% ggplot(aes(velocity, group = condition, fill = condition)) + 
  geom_density(#color = "condition",
    #fill = "#800000",
    alpha = 0.5) + 
  labs(y = "Density",
...
## Accuracy
visyn %>% ggplot(aes(accuracy, group = condition, fill = condition)) + 
...
## Release Time
grid.arrange(g, h, widths = c(3,3), heights = c(3)) ### FIGURE 1
grid.arrange(i, j, widths = c(3,3), heights = c(3)) ### FIGURE 1
